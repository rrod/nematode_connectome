...
////////////////////////////////////////////////////////////////////////////////
DESCRIPTION
This page is the front-end that the user interacts with.  Ajax queries are
called via Javascript to the PHP back-end (query.php) in order to make SQL
queries.  These queries are output onto this page, where they are then parsed
via Javascript into useful information.
////////////////////////////////////////////////////////////////////////////////
-->
...
<!--
The "controls_background" div serves as the background forthe text display
and controls.
-->
<div id="controls_background" class='controls_background'></div>


<!-- The "view" div contains the canvas elements. -->
<div id="view" class='view'>
  <!-- Canvas for the cell bodies (soma) and legend. -->
  <canvas id="canv" class="canv" width="1000" height="300"
    onclick="get_canv_click()"></canvas>
  <!-- Canvas for the cell projections (dendrites & axons; represented as
    single lines). -->
  <canvas id="canv_2" class="canv_2" width="1000" height="300"></canvas>
</div>


<!--
The "choose" div is where the controls are stationed, which themselves are
generated in the javascript below.
-->
<form id="choose_2" action='#' onsubmit="handle()">
<div id="choose" class="choose">Neuron-ID#</div>
</form>

<div id="cell_name_1" class="cell_name_1">Cell Name</div>

<div id="cell_location_1" class="cell_location_1">Location</div>

<div id="cell_type_1" class="cell_type_1">Cell Type</div>

<!-- [NYI] - Individual cell function. -->
<div id="cell_function_1" class="cell_function_1">Function</div>
...
<!-- LEGEND -->
<div id="legend_2" class="legend_2">
...
<!-- HELP menu -->
<div id="help" class="help" onclick="help_menu(1)">
  HELP
</div>

<div id="help_menu" class="help_menu"></div>
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// This AJAX code snippet submits the requests to query.php, which in turn
// uses PHP to submit SQL queries to the SQL database that holds the connectome
// data that was generated in R.
...
function help_menu(value){
  if (value == 1){
    ...
    help_ID.innerHTML = "\
    <a href='#' onclick='help_menu(0)'\
    style='position:absolute; top:0%; right:0%; background:#555555;'>CLOSE</a>\
    HELP MENU NYI\
    <hr>\
    "
  } else{
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// Clears and re-draws the lowest level canvas ("canv_2") when a neuron is
// selected.  This is where the neural projections are drawn.
function handle(e){
        clear_screen()
        select_neuron()
        return false;
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// This generates a drop down menu which would contain available neurons to
// select from.  It is currently unused but may be used in the future.
...
// This generates a number box that the user can input a number into which
// corresponds to the ID of a neuron, which would then generate the information
// about that particular neuron.
var x = document.createElement("input")
    ...
    // because JS arrays start at "0" but the SQL IDs start at "1"
    x.value = y_2.index + 1;

// This creates a button that is used to generate the neuron data from the
// value of the number box above.
...
function select_neuron(){
  // calls to the AJAX function in the header script to grab the SQL data
  sql_query(x.value);
  // identifies that the relevant neuron is now selected
  neuron[x.value].cell_selected = 1;
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// Clears the underlying canvas ("canv_2") which is used for drawing
// the projections.
...
// because the div only covers 80% of the height
canv.height = document.body.clientHeight * 0.80;
canv_2.width = document.body.clientWidth;
// because the div only covers 80% of the height
canv_2.height = document.body.clientHeight * 0.80;
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// Draws neural projections where value_1 = pre-synaptic, and
// value_2 = post-synaptic.
...
// selected = determines if the cell is currently being viewed (selected = 1)
// or not (selected = 0), x_pos = horizontal position of cell body (soma) on
// the canvas ("canv"), y_pos = vertical position of the cell body (soma) on
// the canvas ("canv") (is actually z-axis) type_color = displays a different
// color per soma based on the neuron type (unknown, sensory neuron,
...
function neuron_struct(id, selected, x_pos, y_pos, type_color){
  ...
  if (selected == '1'){
    if (old_neuron != x.value){
      ...
      draw_neuron(neuron[old_neuron].cell_x, neuron[old_neuron].cell_y, "#0FF")
    }
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// [NYI] - Intended to help with selecting neurons by clicking on their cell
// bodies (soma) on the div element ("canv").
function get_canv_click(value1, value2){
  // get x/y coords of mouse event
}
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
function draw_neurons(value){
  ...
  // Shifts the cell bodies over to the right for easier visualization.
  soma_pos_x = soma_pos_x + (canv.width * 0.10);

  var soma_pos_y = canv.height * (search_z * 0.01);

  neuron[value] =
    new neuron_struct(value, 0, soma_pos_x, soma_pos_y, type_color);
}
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// These variables are used in the earlier function draw_neurons().
// These variables hold the actual values of the neuron cell bodies (soma)
// which are then adjusted in the draw_neurons() function for better
// visualization.
var search_x = 0
var search_z = 0

function sql_parse(){
  // These are the strings for which the locations of the cell bodies are
  // located.  They are from the SQL queries and are here to be parsed via
  //  Javascript before they are used.
  ...
  for (var i = 1; i < neuron_names.length; i++){
    // X-COORDINATES ///////////////////////////////////////////////
      ...
      var X_coord_triple_string =
        new RegExp(" " + i + "_" + "[0-9]" + "[0-9]" + "[^,]", 'g');
      var X_coord_triple_1 = X_locations.match(X_coord_triple_string)

      // double digit X value
      var X_coord_double_string =
        new RegExp(" " + i + "_" + "[0-9]" + "[^,]", 'g');
  ...
  // Deletes the data so from this page so that it doesn't interfere with
  // the display.
  ...
  for (var i = 1; i < neuron_names.length; i++){
    if (string_of_projections.match(neuron_names[i])){
      ...
      search_string_of_projections =
        neuron_names[i] + "_" + string_of_projections.match(/\Bgap/i);
      ...
      search_string_of_projections =
        neuron_names[i] + "_" + string_of_projections.match(/\Breceive/i);
      if (string_of_projections.match(search_string_of_projections)){
      ...
      search_string_of_projections =
        neuron_names[i] + "_" + string_of_projections.match(/\Bsend/i);
      if (string_of_projections.match(search_string_of_projections)){
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// List of every neuron in the C. elegans hermaphrodite connectome.
// This list is used to parse the SQL queries from the PHP back-end.
var list_of_neurons =
["","ADAL","ADAR","ADEL","ADER","ADFL","ADFR","ADLL","ADLR","AFDL","AFDR",
"AIAL","AIAR","AIBL","AIBR","AIML","AIMR","AINL","AINR","AIYL","AIYR","AIZL",
"AIZR","ALA","ALML","ALMR","ALNL","ALNR","AQR","AS1","AS2","AS3","AS4","AS5",
"AS6","AS7","AS8","AS9","AS10","AS11","ASEL","ASER","ASGL","ASGR","ASHL","ASHR",
"ASIL","ASIR","ASJL","ASJR","ASKL","ASKR","AUAL","AUAR","AVAL","AVAR","AVBL",
"AVBR","AVDL","AVDR","AVEL","AVER","AVFL","AVFR","AVG","AVHL","AVHR","AVJL",
"AVJR","AVKL","AVKR","AVL","AVM","AWAL","AWAR","AWBL","AWBR","AWCL","AWCR",
"BAGL","BAGR","BDUL","BDUR","CANL","CANR","CEPDL","CEPDR","CEPVL","CEPVR","DA1",
"DA2","DA3","DA4","DA5","DA6","DA7","DA8","DA9","DB1/3","DB2","DB3/1","DB4",
"DB5","DB6","DB7","DD1","DD2","DD3","DD4","DD5","DD6","DVA","DVB","DVC","FLPL",
"FLPR","HSNL","HSNR","I1L","I1R","I2L","I2R","I3","I4","I5","I6","IL1DL",
"IL1DR","IL1L","IL1R","IL1VL","IL1VR","IL2DL","IL2DR","IL2L","IL2R","IL2VL",
"IL2VR","LUAL","LUAR","M1","M2L","M2R","M3L","M3R","M4","M5","MCL","MCR","MI",
...
"RMER","RMEV","RMFL","RMFR","RMGL","RMGR","RMHL","RMHR","SAADL","SAADR","SAAVL",
"SAAVR","SABD","SABVL","SABVR","SDQL","SDQR","SIADL","SIADR","SIAVL","SIAVR",
"SIBDL","SIBDR","SIBVL","SIBVR","SMBDL","SMBDR","SMBVL","SMBVR","SMDDL","SMDDR",
"SMDVL","SMDVR","URADL","URADR","URAVL","URAVR","URBL","URBR","URXL","URXR",
"URYDL","URYDR","URYVL","URYVR","VA1","VA2","VA3","VA4","VA5","VA6","VA7","VA8",
"VA9","VA10","VA11","VA12","VB1","VB2","VB3","VB4","VB5","VB6","VB7","VB8",
"VB9","VB10","VB11","VC1","VC2","VC3","VC4","VC5","VC6","VD1","VD2","VD3","VD4",
"VD5","VD6","VD7","VD8","VD9","VD10","VD11","VD12","VD13"];
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// Re-naming the list of neurons to adjust for list_of_neurons being 303
// elements instead of the 302 actual elements.  It goes back to the issue of
// the SQL neuron ID numbers starting at "1" instead of "0" as a JS array would.
// As you can see in the list_of_neurons array, the first element is left blank,
 // corresponding to no actual neuron existing in the SQL array for "0."
var neuron_names = []
for (var i = 1; i < 302; i++){
